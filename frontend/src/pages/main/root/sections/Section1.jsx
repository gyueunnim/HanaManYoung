...
function MainTitle() {
  ...
  return (
    <h2 className="font-bold flex items-center">
      {user.user_name}님이 필요할 것 같아 준비했어요
      <img
...
function LoginBox({ setIsLogin }) {
  ...
  useEffect(() => {
    const handleMessage = async (event) => {
      const { success, userId, jwt } = event.data; // jwt도 같이 받아옴
    ...
    return () => window.removeEventListener("message", handleMessage); // 이벤트 리스너 제거
